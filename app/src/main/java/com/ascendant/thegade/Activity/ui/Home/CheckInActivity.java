...
//                        Toast.makeText(CheckInActivity.this, "Berhasil", Toast.LENGTH_SHORT).show();
                        LocalDate dates = LocalDate.now();
                        LocalTime time = LocalTime.now();
                        Absen(dates+" "+AscNet.TimeChanger(time.toString()));
                    }else{
                        Toast.makeText(CheckInActivity.this, "Diluar Jangkauan", Toast.LENGTH_SHORT).show();
    ...
    private void Absen(String Waktu){
        final ProgressDialog pd = new ProgressDialog(CheckInActivity.this);
        pd.setMessage("Proses Absensi");
        pd.show();
        pd.setCancelable(false);
        ApiRequest api = RetroServer.getClient().create(ApiRequest.class);
        final Call<ResponseObject> Absen =api.CheckIn(AscNet.AUTH(),Id,Waktu,latitude.getText().toString(),longitude.getText().toString());
        Absen.enqueue(new Callback<ResponseObject>() {
            @Override
            public void onResponse(Call<ResponseObject> call, Response<ResponseObject> response) {
                pd.hide();
                try {
                    Toast.makeText(CheckInActivity.this, response.body().getMessage(), Toast.LENGTH_SHORT).show();
                }catch (Exception e){
...
//                    Toast.makeText(CheckInActivity.this, "Terjadi Kesalahan", Toast.LENGTH_SHORT).show();
                }
            }
